...
G4VPhysicalVolume *MyDetectorConstruction::Construct()
{
    ...
    auto solidleakfrontrear = new G4Box("solidFrontRear",XSize/2,YSize/2,leakthick/2);
    auto solidleaksides = new G4Box("solidleaksides",leakthick/2,YSize/2,large/2);
    auto solidleaktop = new G4Box("solidleaktop",XSize/2,leakthick/2,large/2);
    ...
    logicleak[0]=new G4LogicalVolume(solidleakfrontrear,G10,"logicFrontRear");
    logicleak[1]=new G4LogicalVolume(solidleaksides,G10,"logicSides");
    logicleak[2]=new G4LogicalVolume(solidleaktop,G10,"logictop");
    
    G4LogicalVolume *SiPMlogic = new G4LogicalVolume(SolidSiPM,SiPM,"SiPMlogic");//SiPM
    ...
    G4VPhysicalVolume *physicalWorld = new G4PVPlacement(0,
                                                        ...
                                                        "mundofisico",
                                                        0,false,0,true);
    G4VPhysicalVolume *frontleak = new G4PVPlacement(0,G4ThreeVector(0,0,-leakthick/2),logicleak[0],"front",logicWorld,false,0,false);
    G4VPhysicalVolume *rearleak =new G4PVPlacement(0,G4ThreeVector(0,0,228.94+leakthick/2),logicleak[0],"rear",logicWorld,false,1,false);

    G4VPhysicalVolume *sideleak1 = new G4PVPlacement(0,G4ThreeVector(XSize/2+leakthick/2,0,large/2),logicleak[1],"side+",logicWorld,false,2,false);
    G4VPhysicalVolume *sideleak2 = new G4PVPlacement(0,G4ThreeVector(-(XSize/2+leakthick/2),0,large/2),logicleak[1],"side-",logicWorld,false,3,false);

    G4VPhysicalVolume *topleak1 = new G4PVPlacement(0,G4ThreeVector(0,(YSize/2+leakthick/2),large/2),logicleak[2],"top+",logicWorld,false,4,false);
    G4VPhysicalVolume *topleak2 = new G4PVPlacement(0,G4ThreeVector(0,-(YSize/2+leakthick/2),large/2),logicleak[2],"top-",logicWorld,false,5,false);
    ...
    logicleak[0]->SetVisAttributes(invisible);
    logicleak[1]->SetVisAttributes(invisible);
    logicleak[2]->SetVisAttributes(invisible);
...
*/
    ...
    visPCB->SetForceSolid(false);  // default: SetForceWireframe(true);
...
void MyDetectorConstruction::ConstructSDandField()
{
    ...
    leakSD *leaker = new leakSD("SensitivesLeakers","LeakersCollections");
    for (G4int i= 0;i<3;i++ )
    {
        logicleak[i]->SetSensitiveDetector(leaker);
    }
    sdManager->AddNewDetector(leaker);
