...
public:
    ...
    inline void SetType(G4int Type);
...
inline void SetCopy(G4int copy); //Aquí se define la función que guarda el tipo -1 EM y 1 HAD

    inline G4double GetCopy() const; //Aquí se define la función que accede al tipo -1 EM y 1 HAD (Esto se usa en el MyEventAction::EndOfEventAction)
...
inline void SciHit::SetEnergy(G4double energy)
    ...
    inline void SciHit::SetType(G4int Type) //Aquí se define la función que guarda el tipo -1 EM y 1 HAD
    {
        fType = Type;
    }

    inline G4int SciHit::GetType()  const
    {return fType;}//Aquí se define la función que accede al tipo -1 EM y 1 HAD (Esto se usa en el MyEventAction::EndOfEventAction)
